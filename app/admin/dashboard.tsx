...
export default function AdminDashboard() {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            // Fetch Users via text search or view? 
            // Better to use the RPC function we created for direct auth access
            const { data: users, error: usersError } = await supabase.rpc('get_all_users');

            if (usersError) {
                console.log('Error fetching users via RPC:', usersError);
                // Fallback or alert
                Alert.alert("Setup Required", "Please run the SQL script 'fix_admin_users.sql' in Supabase SQL Editor to see users.");
            }

            // Calculate Stats
            const totalRevenue = orders?.reduce((sum, order) => sum + (order.total_amount || 0), 0) || 0;
            const totalOrders = orders?.length || 0;
            const uniqueUsersCount = users ? users.length : (new Set(orders?.map(o => o.user_id)).size);
    ...
    const formatDate = (dateString: string) => {
        ...
        if (diffInSeconds < 172800) return `${Math.floor(diffInSeconds / 3600)}h ago`; // Extended to 48h to prefer relative time or stick to 24h? User asked for specific format.

        // Use requested 'dd mmm yyyy' format
